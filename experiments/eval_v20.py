#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""在 Roboflow v20 快照上评估 best_model.pth（CNN × 几何 RF 融合）。

结构照搬 eval_live.py：RF 用 v20/train 的几何特征训练，测试集为 v20/test。
"""
...
def main() -> None:
    ...
    P, G = [], []
    tst = ROOT / "test" / "labels"
    files = sorted(tst.glob("*.txt"))
    ...
    print("\n预测分布:", Counter(KEEP[i] for i in P.tolist()))
