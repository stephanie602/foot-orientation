...
def main(SPLIT="test", NEWDIR="v19_current") -> None:
    ...
    lbls = sorted(Path(f"v19/{SPLIT}/labels").glob("*.txt"))
    for n, lbl in enumerate(lbls, 1):
        ip = Path(f"v19/{SPLIT}/images") / (lbl.stem + ".jpg")
        new_lbl = Path(f"{NEWDIR}/{SPLIT}/labels") / lbl.name
    ...
    print(f"\n评估样本 {len(p)}\n")
    for tag, gt in (("A. 原始人工标注（推送前）", a), ("B. 当前 Roboflow 状态（纵轴模型 / 横轴人工）", b)):
        acc, f1 = metrics(gt, p)
        print(f"{tag}")
